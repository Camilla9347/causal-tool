...
The data processing procedure described in (Pelagalli et al, in preparation) is a post-processing of the expansion lists generated by OneGenE that aims at orienting the greatest possible number of direct associations found between the gene of interest (seed) and the genes of its expansion list, through the application of the whole PC algorithm. This procedure builds an _in silico_ model of regulatory gene networks.

Once the gene of interest is chosen, a subsetting of its expansion list is carried out: the gene of interest is selected together with a certain number of top ranking genes in the expansion, chosen by the user. The expression data relative to this subset of genes are extracted from the transcriptomic dataset under examination. The PC algorithm receives as input a data matrix, experiments x genes, and produces as output a partially directed graph in which the nodes represent the genes and the directed edges the found cause-effect relationships. This graph can serve as a test for already-known functional dependencies between genes or it can be a tool for the biologist to discover and further investigate new interactions.

## Prerequisites
To run the tool, make sure you have installed, at least in the **causal-tool-main** folder of this repository, **R version 4.4.3**, used to write the procedure, with **R packages** **pcalg_2.7-12** and **readr_2.1.5**. For more information check the **Reproducibility** section below.
...
3. The expansion list will appear in your **Downloads** folder as a zip compressed folder. Unzip the folder to get access to the corresponding **.csv** file (_T105641_p1@<!-- -->U2AF1.csv_)
4. Move the expansion list to the **causal-tool-main** folder of this project to provide it as input to the ***causal tool***.
...
Right now, the file _fantom_mat.csv_ is a placeholder for the actual FANTOM-full transcriptomic dataset, a gene@home version of the FANTOM5 transcriptomic dataset. This file should be replaced and renamed accordingly in order for the procedure to work. The FANTOM-full transcriptomic dataset can be downloaded from: [Human OneGenE download page](https://gene.disi.unitn.it/test/download/bc/hgnc_data_mat.csv.gz). The file will need to be extracted, renamed (_fantom_mat.csv_) and placed in the **causal-tool-main** folder.

### Vitis vinifera

Right now, the file _vespucci_mat.csv_ is a placeholder for the actual VESPUCCI transcriptomic dataset. This file should be replaced and renamed accordingly in order for the procedure to work. The VESPUCCI transcriptomic dataset can be downloaded from: [Vitis OneGenE download page](http://ibdm.disi.unitn.it/download-2/). The file will need to be extracted, renamed (_vespucci_mat.csv_) and placed in the **causal-tool-main** folder.


## Input submission

To run the causal tool, make sure you have a terminal panel open in the **causal-tool-main** folder and type the following command:
...
These two files are contained in the **Vv** folder inside the **causal-tool-main** folder, if a grapevine expansion list was chosen as input to the tool, or they are contained in the **Hs** folder inside the **causal-tool-main** folder, if a human expansion list was chosen as input.


To visualize the pc() output graph on Cytoscape, do the following steps:

1. Open Cytoscape and allow the app to accept incoming network connections;
2. Select the _network_ icon from the main horizontal toolbar, which stands for _Import Network from File System_ (or from File -> Import -> Network from file) and select the _\_edges.csv_ file from the **Vv** folder or **Hs** folder inside **causal-tool-main**;
3. Click the OK button in the _Import Network from Table_ panel and wait for the network to load;
4. Select the _table_ icon from the main horizontal toolbar, which stands for _Import Table from File_ (or from File -> Import -> Table from file) and select the _\_nodes.csv_ file from the **Vv** folder or **Hs** folder inside **causal-tool-main**;
...
6. The **Vv** and **Hs** folders contain respectively a _Vv_style.xml_ and a _Hs_style.xml_ that can be uploaded in Cytoscape to customize the network appearance (From File -> Import -> Styles from file...). This feature is managed by the _Style_ panel (under _Network_ in the main vertical toolbar), from which you can select the uploaded style and visualize the network in a more human-friendly and enriched way.

## Reproducibility

The relevant information from the R ```sessionInfo()``` command is presented here to reproduce the output presented in the paper, using the most updated version of the **pcalg** package:
